...
const char *server_pass_url = "http://192.168.137.88/api/password.php";
...
void syncFromServer()
{
  ...
  // 🔐 1. Đồng bộ mật khẩu từ server_pass_url
  HTTPClient http_pass;
  http_pass.begin(server_pass_url); // Endpoint: save_password.php
  http_pass.addHeader("Content-Type", "application/x-www-form-urlencoded");

  int code_pass = http_pass.POST("action=get_password");

  if (code_pass == 200)
  {
    String pass_str = http_pass.getString();
    Serial.println("🔑 Mật khẩu từ server: " + pass_str);

    bool pass_diff = false;
    for (int i = 0; i < 5; i++)
    {
      if (EEPROM.read(i) != pass_str[i])
      {
        pass_diff = true;
        break;
      }
    ...
    if (pass_diff)
    {
      ...
      for (int i = 0; i < 5; i++)
      {
        EEPROM.write(i, pass_str[i]);
        password[i] = pass_str[i];
      }
      EEPROM.commit();
    }
    else
  ...
  {
    Serial.print("❌ Lỗi lấy mật khẩu, mã lỗi: ");
    Serial.println(code_pass);
  }
  http_pass.end();

  // 🪪 2. Đồng bộ RFID từ server_rfid_url
  HTTPClient http_rfid;
  http_rfid.begin(server_rfid_url); // Endpoint: rfid.php
  http_rfid.addHeader("Content-Type", "application/x-www-form-urlencoded");

  int code_rfid = http_rfid.POST("action=sync");

  if (code_rfid == 200)
  {
    String payload = http_rfid.getString();
    ...
    if (error)
    {
      Serial.println("❌ Lỗi phân tích JSON RFID");
      http_rfid.end();
      return;
    }
    ...
    EEPROM.commit();
    Serial.println("✅ Đồng bộ RFID hoàn tất!");
    checkEEPROM();
  }
  else
  {
    Serial.print("❌ Lỗi sync RFID, mã lỗi: ");
    Serial.println(code_rfid);
  }
